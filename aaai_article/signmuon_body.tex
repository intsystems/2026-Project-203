...
We exhibit such a gradient for each placement. For SignMuon it is a $\mathbf{G}\in\mathbb{R}^{4\times4}$ built as $1000\,\mathbf{u}_1\mathbf{v}_1^\top+\mathbf{O}$ with $\mathbf{O}$ orthogonal, so that $\operatorname{polar}(\mathbf{G})=\mathbf{O}$ while the sign pattern of $\mathbf{O}$ is anticorrelated with the dominant component, giving $\langle\mathbf{G},\mathbf{s}(\mathbf{G})\rangle=-\tfrac{42468}{103}$. Signing \emph{before} the oracle fares no better --- the spectral LMO can rotate a sign matrix away from $\mathbf{G}$ rather than heal it --- and a single $\mathbf{G}\in\mathbb{R}^{5\times5}$ defeats both remaining placements at once, at $\langle\mathbf{G},\mathbf{s}(\mathbf{G})\rangle\approx-13.89$ for MuonUSign and $-76$ for MuonSign. These are Theorems~\ref{th:1}--\ref{th:3}, with the constructions in Appendices~\ref{app:proof_th1}--\ref{app:proof_th3}; the left and centre panels of Figure~\ref{fig:divergence_plot} show the resulting ascents.
...
\paragraph{Protocol.} The only tuned hyperparameter is $\eta_0$, selected per method on a held-out $5$k validation split; \textbf{the test set is never consulted during tuning}. Every method with a norm-fixed step is tuned and reported under one per-layer rule, the unit-gain rule~\eqref{eq:unit_gain_main}, so that $\eta_0$ means the same thing --- the per-step RMS gain --- across them (Appendix~\ref{app:lrscale}); SGD and Adam have no such step and keep one global rate. Centralized numbers are means over three seeds and federated numbers over five, and differences smaller than the seed spread are not claimed as results. Alongside final accuracy we report epochs to a fixed test accuracy, which separates the methods by factors rather than by tenths of a point. The grid, the endpoint rule and the schedules are in Appendix~\ref{app:repro}.
