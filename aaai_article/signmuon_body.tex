...
\paragraph{Exact zeros.} We resolve $\operatorname{sign}(0)$ to an independent random $\pm1$ rather than to $0$, on every channel in this paper: a third symbol would make the alphabet ternary and the channel wider than one bit, whereas randomizing incurs no cost in expected descent and makes exact two statements the ternary convention only approximates: $\|\mathbf{s}_t\|_F=\sqrt{mn}$, which the step-size rule below assumes, and the contraction identity of Lemma~\ref{lem:signcontr} (Appendix~\ref{app:hyp}). Everything below is stated under this convention, but none of the counterexamples invokes it: no instance in Theorems~\ref{th:1}--\ref{th:ef_div} has a zero entry.
...
For matrix parameters the key component is the orthonormal LMO direction $\mathbf{D}_t \approx \mathbf{U}_t \mathbf{V}_t^\top$, which we approximate by a 5th-order Newton--Schulz iteration rather than a full SVD at every step (Algorithm~\ref{alg:muon_lmo}). SignMuon signs that direction, so it transmits one bit per component while keeping the geometry of orthonormal updates (Algorithm~\ref{central_alg} in Appendix~\ref{app:alg}); MuonSign is built from the same idea (Algorithm~\ref{alg:muon_sign}). One further implementation choice is not without consequence, and we state it here because every experiment depends on it.
...
Error feedback on the compressed \emph{output}, however, does not restore the descent property. It fails \emph{universally}, at every problem scale and momentum setting.
