...
\end{aligned}\)}
\end{equation}
where $\mathbf{X}_t \in \mathbb{R}^{m \times n}$ is the parameter matrix, $\mathbf{G}_t$ its stochastic gradient, $\mu \in [0, 1)$ the momentum coefficient and $\eta_t$ the learning rate; $A(\cdot)$ is a Linear Minimization Oracle (LMO) in a specified non-Euclidean norm, and $\operatorname{sign}(\cdot)$ acts element-wise on the structured direction $\mathbf{D}_t$, with $\operatorname{sign}(0)$ resolved to an independent random $\pm1$ so that the transmitted alphabet stays binary; randomized sign operators are standard here \citep{chen2020median,safaryan2021ssdm,jin2024stosign}.
