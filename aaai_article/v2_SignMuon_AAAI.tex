...
$$ \mathbf{D}_t^{(j)} = -A\bigl(\mathbf{M}_t^{(j)}\bigr), \qquad \mathbf{s}_t^{(j)} = \operatorname{sign}\bigl(\mathbf{D}_t^{(j)}\bigr) $$
...
= \epsilon\,C + M\,D_{4,2},
\end{equation}
where $C:=\sum_{(i,j)\neq(4,2)}|D_{i,j}|=10.366>0$ is fixed (every such entry agrees in sign with $\mathbf{S}$), while $M\,D_{4,2}=-0.2425\,M\to-\infty$. Thus $\langle\mathbf{G},\mathbf{D}\rangle<0$ for any $M>C\epsilon/0.2425$; with $\epsilon=1,\ M=100$ one obtains $\langle\mathbf{G},\mathbf{D}\rangle=-13.89$, for the exact polar factor that the theorem is stated over. By Proposition~\ref{prop:reduction}, MuonUSign strictly ascends on $f(\mathbf{W})=\langle\mathbf{G},\mathbf{W}\rangle$ for every $\eta_t>0$, every $\mu\in[0,1)$, and both momentum variants; $f(\mathbf{X}_t)\to+\infty$ under any non-summable step size. $\blacksquare$
