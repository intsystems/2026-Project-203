...
\begin{aligned}
\text{\textsc{lmo} family } &(\text{Muon, MuonUSign, EF21-Muon}\{\text{USign},\text{Sign}\},\ \text{EF21-SignMuon}): &&\mathbf{s}=\mathbf{U}\mathbf{V}^\top, \\
\text{\textsc{sign} family } &(\text{SignMuon, MuonSign, SignSGD}): &&\mathbf{s}\in\{\pm1\}^{m\times n}.
\end{aligned}
\end{equation}
...
Muon default & $\sqrt{\max(1,\tfrac{m}{n})}$ & $1$ & \textsc{lmo} gain only \\
\textbf{unit gain} & $\sqrt{\max(1,\tfrac{m}{n})}$ & $n^{-1/2}$ & per-step gain, both families \\
$\mu$P ($\alpha\!=\!1$) & $\sqrt{\max(1,\tfrac{m}{n})}$ & $n^{-1}$ & gain of the aligned accumulation \\
\citet{mishra2026signmuon} & $r^{-1/2}$ & $(mn)^{-1/2}$ & $\|\mathbf{s}\|_F$ (their analysis) \\
\bottomrule
\end{tabular}
\caption{Per-layer step-size multipliers $\lambda_\ell$. Only $\eta_0$ is tuned; $\lambda_\ell$ is fixed a priori by the shape. The rules differ only in the \textsc{sign} family, and the \textsc{lmo} column of the unit-gain rule coincides with the factor already used in practice \citep{jordan2024muon} -- which is our main evidence that \eqref{eq:gain_def} is the right criterion.}
\label{tab:lrscale}
\end{table}
