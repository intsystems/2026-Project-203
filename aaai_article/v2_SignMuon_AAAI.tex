...
= \mathcal{O}(T^{-1/2}),
\end{equation}
as well as an $\mathcal{O}(T^{-1/4})$ rate for $\min_t \mathbb{E}\|\nabla f(\mathbf{X}_t)\|_*$ under the weaker layer-wise $(L^0,L^1)$-smoothness with a plain constant learning rate (Corollary~\ref{cor:l0l1}). Finally, EF21-MuonSign adds a second error-feedback loop on the downlink, making communication 1-bit in \emph{both} directions. The $\mathcal{O}(T^{-1/2})$ rate survives, but not for free: a sign-based downlink is contractive only in the Euclidean norm, whereas Muon's spectral geometry asks for contractivity in the layer norm, so the admissible step size shrinks by a further factor of the layer rank. We show that this gap cannot be closed for the scaled sign by any sharper analysis -- only by changing the compressor (Remark~\ref{rem:normmismatch}) -- and we measure what it costs on a transformer (Section~\ref{sec:exp_nanogpt}).
...
The LMO orthogonalization is moved from the clients to the server: each client evaluates the difference between its local momentum $\mathbf{M}_t^{(j)}$ and the previous gradient estimate $\mathbf{g}_t^{(j)}$, compresses it via scaled sign, and sends only $(\mathbf{s}_t^{(j)}, \alpha_t^{(j)})$. The server aggregates these, updates a global gradient estimator $\mathbf{G}_{t+1}$, and applies the Muon LMO. The complete Algorithm~\ref{alg:fed_serverlmo} is in Appendix\ref{app:alg}. The transmission of an additional scalar $\alpha_t^{(j)}$ for each layer introduces insignificant communication overhead, keeping the uplink communication cost at $\approx 1$ bit per parameter (the server broadcasts the full model on the downlink). Thus, all federated variants -- SignMuon, EF21-MuonUSign and EF21-MuonSign -- effectively address: the baseline majority-vote scheme and the error-feedback variant -- effectively address the communication bottleneck in federated learning. They preserve the geometric benefits of Muon-style matrix orthogonalization while maintaining an extremely low communication budget.
...
Before the network experiments we verify on a deterministic $L$-smooth convex quadratic that sign compression of an \textsc{lmo} step preserves the structural advantage: SignMuon reaches the target in $2104$ iterations against SignSGD's $3120$, at an identical one-bit budget, while the error-feedback variants pay the overhead their guarantee costs on a problem benign enough not to need it. The construction, the grid search and the trajectories are deferred to Appendix~\ref{app:images_task}.
...
\paragraph{Protocol.} Every method is tuned and reported under one per-layer rule, the unit-gain rule of Appendix~\ref{app:lrscale}, so that $\eta_0$ means the same thing --- the per-step RMS gain --- for all eleven and no comparison is confounded by the scaling convention. The only tuned hyperparameter is $\eta_0$. Momentum is fixed at $0.9$ and weight decay at $0$ for the primary table: that is the setting our theorems analyse, the one the reference nanoGPT configuration uses, and the one \citet{mishra2026signmuon}'s own sweep selects; the regularized case is an ablation (Appendix~\ref{app:images_cen}). The auxiliary group --- embeddings, normalization parameters, the classifier head --- is AdamW at a fixed $10^{-3}$ for every method, so the matrix rule is the only thing that varies.
...
Alongside final accuracy we report the number of epochs to reach a fixed test accuracy. The two measure different things: with eleven methods within roughly a point of one another at $75$ epochs, final accuracy is close to the noise floor, whereas a threshold crossing on a monotonically rising curve separates the methods by factors rather than by tenths of a point, and is the analogue of the ``steps to $3.35$'' column of Table~\ref{tab:nanogpt}.
...
The results of the grid search with the number of iterations required to converge to 0.001, are summarized in Table~\ref{tab:synthetic_results}. Muon converges in 1122 iterations, comparable to SGD (972) but slower than Adam (202). SignMuon reaches the target in 2104 iterations, significantly outperforming SignSGD (3120). EF21-MuonUSign requires 3694 iterations and EF21-MuonSign requires 3564 iterations; its overhead on this benign convex problem is expected, since its purpose is to guarantee convergence on problems where SignMuon does not converge (Theorem~\ref{th:1}). The optimization trajectories are shown in Appendix\ref{app:images_task}, Figure~\ref{fig:synthetic_results}.
...
\caption{The downlink measurement for EF21-MuonSign, per layer type: the contraction $\alpha(\boldsymbol{\Delta}^{\downarrow})$ the scaled sign actually achieves (solid) and the resulting server/broadcast gap $\|\mathbf{X}_t-\mathbf{W}_t\|_F/\|\mathbf{W}_t\|_F$ (dashed). On the zero-initialized $\mathtt{c\_proj}$ the contraction collapses by three orders of magnitude and the gap stalls near $10^{-1}$; every other layer type contracts at $\Theta(10^{-1})$ and closes its gap by ${\sim}65\times$ over the run. This is Remark~\ref{rem:dimension} measured.}
...
\paragraph{The one open exponent.} Writing $\lambda_\ell^{\textsc{sign}}=n^{-\alpha}$, the unit-gain rule is $\alpha=\tfrac12$. Identity \eqref{eq:gain_identity} requires the input to be independent of $\mathbf{A}$, which is the right model for a \emph{single} step; if the \emph{accumulated} update $\sum_t\eta\mathbf{s}_t$ correlates with the activations, its gain can be as large as $\|\mathbf{A}\|_{\mathrm{op}}\sqrt{n/m}=\Theta(\eta n)$ rather than $\Theta(\eta\sqrt{n})$, giving $\alpha=1$ -- the classical $\mu$P rule $\eta\propto 1/\text{fan-in}$ for sign-like (Adam-type) updates \citep{yang2021tuning}. Thus $\alpha\in[\tfrac12,1]$ interpolates between an incoherent and a fully aligned accumulation, and the endpoint is an empirical question rather than a modelling choice. Two measurements bear on it. At the single-step level we find $\|\operatorname{sign}(\operatorname{polar}(\mathbf{M}))\|_{\mathrm{op}}=0.93\,(\sqrt{m}+\sqrt{n})$ to within $8\%$ at every ResNet-18 layer shape, i.e.\ a single sign step is spectrally \emph{incoherent}, not rank-one aligned. The second measurement is direct: we track the realized gain $\|\mathbf{X}_t-\mathbf{X}_0\|_F/\sqrt{m}$ of the accumulated update during training and fit its growth exponent in $t$, which separates the two regimes without reference to accuracy. The fit must be taken at a \emph{constant} step size. Under the cosine schedule of our main runs the accumulation saturates and the exponent measures the schedule rather than the alignment: the same diagnostic on scheduled runs returns $-0.01$ for Muon and $-0.06$ for SignMuon, with $R^2\le0.31$ --- not a sub-diffusive regime but an absence of any power law, which is the signature of a decaying step size and the reason the constant-rate control is required. At a constant rate over $20$ epochs the exponent is $0.513$ for Muon and $0.516$ for SignMuon, at $R^2=1.000$ in both cases.

Two features of that result matter more than its proximity to $\tfrac12$. First, the fit is essentially exact over a decade of $t$, so the accumulation follows a clean power law rather than an averaged trend. Second, the exponent is the \emph{same} for Muon and SignMuon --- and Muon is an \textsc{lmo}-terminated method, for which \eqref{eq:unit_gain_closed} and the $\mu$P rule prescribe the identical multiplier $\sqrt{\max(1,m/n)}$. Muon therefore acts as a control: it fixes the exponent the diagnostic reports when the per-layer rule is not in question, and SignMuon agreeing with it to $0.003$ says the sign family's accumulation is no more aligned than the orthogonalized family's. The measured value coincides with the prediction of the exact single-step identity \eqref{eq:gain_identity} under the independence hypothesis, so the accumulation is incoherent, $\alpha=\tfrac12$, and the $\mu$P endpoint $\alpha=1$ is excluded by the data rather than merely not required by the model. We adopt $\alpha=\tfrac12$ throughout, for both families and for every experiment in this paper.

A caution on what this does \emph{not} settle. The exponent governs how the accumulated update scales with $t$, which is what fixes the shape dependence of $\lambda_\ell$ and hence the transfer of $\eta_0$ across widths. It does not assert that $\alpha=\tfrac12$ maximizes accuracy at any one width. At a single width the exponent is largely absorbed into $\eta_0$: sweeping $\alpha\in\{0,\tfrac12,1\}$ with $\eta_0$ retuned for each, on the ResNet-18 of Section~\ref{sec:exp_nanogpt}'s companion experiment, moves validation accuracy by less than $0.3\%$ --- inside the seed spread, and hence not a discriminating test. This is expected: $\alpha$ changes only the \emph{relative} rates across layers, and ResNet-18's fan-in range is narrow enough that the profile barely tilts. The accumulated-gain exponent is the measurement that separates the hypotheses; the accuracy sweep is reported for completeness and is consistent with either.

\paragraph{Weight decay is not free to place.} The same scale invariance that makes $\lambda_\ell$ necessary also dictates \emph{where} an $\ell_2$ penalty may be applied. Every step map considered here is positively homogeneous of degree \emph{zero}: $\operatorname{sign}(c\mathbf{M})=\operatorname{sign}(\mathbf{M})$ and $\operatorname{polar}(c\mathbf{M})=\operatorname{polar}(\mathbf{M})$ for all $c>0$. Consequently, folding a decay term into the gradient before the \textsc{lmo} -- $\widetilde{\mathbf{G}}_t=\mathbf{G}_t+\lambda_{\mathrm{wd}}\mathbf{W}_t$, the convention of \citet{mishra2026signmuon} and of most sign-method implementations -- cannot shrink $\mathbf{W}_t$ at all: the step length $\eta_t\lambda_\ell\|\mathbf{s}\|_F$ is fixed by \eqref{eq:two_families}, and the decay term acts only by \emph{rotating} the direction. For the sign-terminated steps the cancellation is exact to machine precision; for the \textsc{lmo}-terminated ones it is exact for the polar factor itself, and the few percent by which a Newton--Schulz approximation of it moves is the iteration's error, not shrinkage. The rotation is governed by the ratio $\rho_t=\lambda_{\mathrm{wd}}\|\mathbf{W}_t\|_F/\|\mathbf{G}_t\|_F$, and it is not benign: averaged over the ResNet-18 layer shapes, $\rho=0.1$ already flips $3.4\%$ of the coordinates of $\operatorname{sign}(\operatorname{polar}(\cdot))$, and $\rho=1$ flips $26\%$, the polar direction retaining cosine similarity only $0.68$ with the undecayed one. Since $\|\mathbf{G}_t\|_F$ falls by orders of magnitude over training while $\|\mathbf{W}_t\|_F$ grows, $\rho_t$ drifts from negligible to $\Theta(1)$; worse for a comparison, it depends on each method's own momentum scale, so one nominal $\lambda_{\mathrm{wd}}$ is a different perturbation for each. Decoupled decay, $\mathbf{W}_{t+1}=(1-\eta_t\lambda_{\mathrm{wd}})\mathbf{W}_t-\eta_t\lambda_\ell\mathbf{s}_t$, is by contrast exactly commensurate with the update under the unit-gain rule: the displacement it contributes has gain $\eta_t\lambda_{\mathrm{wd}}\gamma(\mathbf{W}_t)$ against the step's gain $\eta_t$, so their ratio is $\lambda_{\mathrm{wd}}\gamma(\mathbf{W}_t)$ -- free of $\eta_t$, of the layer shape, and of the method. We therefore decouple, and use the coupled form only for an ablation. This also explains an otherwise puzzling observation of \citet{mishra2026signmuon}: sweeping $\lambda_{\mathrm{wd}}\in\{0,0.1,0.2\}$ coupled, all ten of their best CIFAR-10 configurations select $\lambda_{\mathrm{wd}}=0$. At their two nonzero values $\rho_t\gg1$ throughout training, so the update degenerates towards $-\operatorname{sign}(\mathbf{W}_t)$; the sweep rejects the placement rather than the regularizer.
