...
Results and selected hyperparameters are in Appendix Table~\ref{tab:cifar_central}; the loss and accuracy dynamics are in Figure~\ref{fig:cifar_results}.
...
\bottomrule
\end{tabular}
\caption{Floor exponent, budget exponents and stability edge. \textbf{[fill from \texttt{SUMMARY.md}, floor / budget scaling / stability sections]} SGD has no floor --- its step vanishes with the gradient --- so its floor columns are empty by construction, and its $\eta_{\max}$ is the control that must reproduce $2/L$.}
...
\begin{figure}[!t]
    ...
    \end{subfigure}
    \caption{\textbf{[placeholder --- generate from \texttt{results/synthetic/}]} Log--log fits behind Table~\ref{tab:synthetic_dynamics}. Left: the accuracy floor of a constant step. Right: best error at each tuned budget.}
...
\hline
\multirow{8}{*}{CIFAR-10} & SignMuon & \multirow{8}{*}{75} & \multirow{8}{*}{128} & 0.001 & 0.0001 & \multirow{8}{*}{0.9} & 99.69\% & 92.55\% \\
...
\cline{2-2} \cline{5-6} \cline{8-9}
& EF21-MuonSign & & & 0.007 & 0.001& & 99.95\% & 93.86\% \\
\hline
\end{tabular}
