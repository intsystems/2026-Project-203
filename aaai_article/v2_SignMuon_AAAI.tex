...
The SignSGD gradient-compression algorithm enables up to a $32\times$ reduction in communication volume, which is critical for federated learning under limited bandwidth, but it ignores the matrix structure of the parameters exploited by modern optimizers such as Muon. A natural attempt to combine the two is to insert a sign operator around the Muon Linear Minimization Oracle (LMO), yet where the sign is placed matters. We study the two resulting one-bit-per-parameter methods: \textbf{SignMuon}, which compresses after the LMO and computes $\operatorname{sign}(\operatorname{LMO}(\cdot))$, and \textbf{MuonSign}, which compresses before the LMO and computes $\operatorname{LMO}(\operatorname{sign}(\cdot))$. We prove that neither enjoys a global convergence guarantee: for each method we construct a smooth linear objective on which it provably diverges (Theorems~\ref{th:1} and~\ref{th:2}). To restore convergence without increasing the communication volume, we introduce \textbf{EF21-MuonUSign}, an Error-Feedback variant in which the scaled-sign operator compresses only the residual between the momentum and a gradient estimator, while the descent step uses the full Muon LMO of the reconstructed estimator on the server; the uplink stays at one bit per parameter and the standard $\mathcal{O}(T^{-1/2})$ rate for smooth non-convex optimization is retained. We further introduce \textbf{EF21-MuonUDSign}, a bidirectional variant that additionally compresses the downlink with the scaled-sign operator through a second error-feedback loop, reducing communication to one bit per parameter in both directions while preserving convergence. Both Error-Feedback methods converge on the same counterexamples where SignMuon and MuonSign diverge. We validate our analysis on a synthetic convex problem and on centralized and federated CIFAR-10 classification.
\end{abstract}
...
We study two natural ways to combine sign compression with the Muon LMO while communicating only one bit per parameter: \textbf{SignMuon}, which places the sign operator \emph{after} the LMO and computes $\operatorname{sign}(\operatorname{LMO}(\cdot))$, and \textbf{MuonSign}, which places it \emph{before} and computes $\operatorname{LMO}(\operatorname{sign}(\cdot))$. Both preserve the matrix-aware geometry of Muon and, empirically, match Muon's accuracy while significantly outperforming SignSGD in centralized and federated learning.

However, neither combination enjoys a global convergence guarantee. We prove that both can diverge: SignMuon on a $4\times4$ counterexample (Theorem~\ref{th:1}) and MuonSign on a $5\times5$ counterexample (Theorem~\ref{th:2}); in each case the 1-bit step becomes an ascent direction. To restore convergence without increasing the communication volume, we introduce \textbf{EF21-MuonUSign}, an Error-Feedback variant adapted from EF21-Muon~\citet{gruntkowska2025error}: the scaled-sign operator compresses only the residual between the momentum and a gradient estimator, while the descent step uses the full Muon LMO of the reconstructed estimator on the server. We further introduce \textbf{EF21-MuonUDSign}, which adds a second error-feedback loop on the downlink to compress both channels to one bit per parameter. Both converge on the counterexamples above while retaining one-bit communication.
...
\\(i)~SignMuon and MuonSign, two matrix-aware 1-bit optimizers that place the sign operator after and before the LMO, respectively;
\\(ii)~counterexamples (Theorems~\ref{th:1}--\ref{th:2}) proving that both may diverge;
\\(iii)~EF21-MuonUSign, which compresses the uplink to one bit per parameter while restoring convergence guarantees;
\\(iv)~EF21-MuonUDSign, a bidirectional variant that adds a downlink error-feedback loop, keeping both channels at one bit per parameter.
...
% \end{equation}
Despite its advantages, Muon requires transmitting the full update matrix $\mathbf{D}_t$, making it expensive in federated settings. Our objective is to combine Muon's matrix-aware geometry with extreme (1-bit) communication compression. SignMuon and MuonSign achieve this combination empirically, but, as we show (Theorem~\ref{th:1} and Theorem~\ref{th:2}), they lack global convergence guarantees. The Error-Feedback variant EF21-MuonUSign restores convergence: the EF21 estimator bounds established in Appendix\ref{app:ef_bounds} provide the key ingredients for the standard $\mathcal{O}(T^{-1/2})$ rate for smooth non-convex optimization (Theorem~\ref{thm:conv}):
\begin{equation}
\min_{0 \le t \le T}\mathbb{E}\bigl[\|\nabla f(\mathbf{X}_t)\|_*^2\bigr]
= \mathcal{O}(T^{-1/2}).
\end{equation}
To complete our theoretical analysis, we introduce EF21-MuonUDSign. This variant utilizes a second error-feedback loop to apply the scaled-sign operator to the downlink. It preserves global convergence guarantees while achieving 1-bit bidirectional communication.
...
The proposed SignMuon algorithm is a particular instance of the SignA family. It combines the orthogonalized LMO directions of the Muon optimizer with the extreme sign-based compression strategy of SignSGD. MuonSign is the mirror image of SignMuon: it applies the sign operator \emph{before} the LMO. Starting from the effective momentum direction $\tilde{\mathbf{M}}_t$, it compresses it to $\operatorname{sign}(\tilde{\mathbf{M}}_t)$ and then takes the Muon LMO of that sign matrix, stepping along the resulting orthonormal direction (Algorithm~\ref{alg:muon_sign}):
\begin{equation}
\mathbf{D}_t = -A\bigl(\operatorname{sign}(\tilde{\mathbf{M}}_t)\bigr), \qquad \mathbf{X}_{t+1} = \mathbf{X}_t - \eta_t\,\mathbf{D}_t.
\end{equation} 
In the following sections, we present SignMuon and MuonSign, each in centralized and federated variants.
...
A natural hope is that the Muon LMO direction--the steepest-descent direction in the spectral norm--remains a valid descent direction even after extreme sign compression. We show that this hope fails: for both placements of the sign operator around the LMO, the compressed step can become an ascent direction.
...
% \end{theorem}
\begin{theorem}[Divergence counterexample]\label{th:1} There exists a smooth function $f(\mathbf{W})$ and a matrix dimension $N_{dim} \ge 4$ such that the SignMuon algorithm diverges for any learning rate $\eta_t > 0$ and any momentum coefficient $\mu \in [0, 1)$, under both Standard and Nesterov momentum variants.
\end{theorem}
...
\subsection{Theoretical limitations of MuonSign}
Like SignMuon, MuonSign communicates only one bit per parameter on the uplink; unlike SignMuon, the step is taken along a full orthonormal LMO direction rather than along its sign. One might therefore hope that the LMO ``heals'' the coarseness of the sign and preserves the descent property. The direction $\mathbf{D}_t$ is a descent direction whenever
\begin{equation}
\bigl\langle \mathbf{G}_t,\, -A\bigl(\operatorname{sign}(\tilde{\mathbf{M}}_t)\bigr)\bigr\rangle > 0,
\label{eq:descent_condition_muonsign}
\end{equation}
yet applying the spectral LMO to a sign matrix can rotate the update away from the gradient, so this inequality need not hold.

\begin{theorem}[Divergence of MuonSign]\label{th:2}
There exists a smooth function $f(\mathbf{W})$ and a matrix dimension $N_{dim} \ge 5$ such that the MuonSign algorithm diverges for any learning rate $\eta_t > 0$ and any momentum coefficient $\mu \in [0, 1)$, under both Standard and Nesterov momentum variants.
\end{theorem}

A proof by counterexample is provided in Appendix\ref{app:proof_th2}: we construct a $5\times 5$ matrix on which MuonSign diverges. Taken together, Theorems~\ref{th:1}--\ref{th:2} establish that neither naive placement of the sign operator around the LMO yields a convergent method, which motivates the Error-Feedback variants introduced next.

\subsection{Centralized Learning with Error Feedback}
% \begin{figure*}[!t]
%     \centering
%     \includegraphics[width=0.45\textwidth]{images/signmuon_divergence_ef.pdf}
...
\begin{figure*}[!t]
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/signmuon_divergence_ef.pdf}
        \caption{}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.3\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/muonsign_divergence_ef.pdf}
        \caption{}
    \end{subfigure}
    \caption{Comparison of optimization trajectories for the linear objective function $f(\mathbf{W}) = \mathrm{Tr}(\mathbf{G}^\top \mathbf{W})$, demonstrating the strict divergence of (a) SignMuon, (b) MuonSign while other methods successfully minimize the function.}
    \label{fig:divergence_plot}
\end{figure*}
...
EF21-MuonUDSign & 0.0028 & 0.1 & 3564 \\
...
The goal of these experiments is empirical validation of the SignMuon, EF21-MuonUSign and EF21-MuonUDSign algorithms. The study consists of several parts, including evaluation in both centralized and federated settings, as well as validation on a synthetic least-squares optimization problem.
...
The results of the grid search with the number of iterations required to converge to 0.001, are summarized in Table~\ref{tab:synthetic_results}. Muon converges in 1122 iterations, comparable to SGD (972) but slower than Adam (202). SignMuon reaches the target in 2104 iterations, significantly outperforming SignSGD (3120). EF21-MuonUSign requires 3694 iterations and EF21-MuonUDSign requires 3564 iterations; its overhead on this benign convex problem is expected, since its purpose is to guarantee convergence on problems where SignMuon does not converge (Theorem~\ref{th:1}). The optimization trajectories are shown in Appendix\ref{app:images_task}, Figure~\ref{fig:synthetic_results}.
...
In this experiment, we compare the convergence dynamics and the final accuracies of SignMuon, EF21-MuonUSign and EF21-MuonUDSign with those of the classical optimizers: Muon, SignSGD, SGD, and Adam at a fixed number of epochs. One of the goals of the experiment is to demonstrate that SignMuon achieves classification quality comparable to Muon, while outperforming the classical SignSGD algorithm under the same uplink communication budget (1 bit per parameter on the uplink).
...
For a systematic analysis, we compare the proposed methods against classical baselines at two federation scales. At each scale we evaluate the proposed SignMuon and the Error-Feedback variants EF21-MuonUSign and EF21-MuonUDSign (Algorithms~\ref{fed_alg}, \ref{ef21_alg} and~\ref{ef21_ud_alg}) against the Muon, SignSGD, SGD, and Adam baselines.
...
\paragraph{Proof} The proof is by counterexample: we exhibit a smooth function and a matrix dimension $N_{dim} \ge 4$ on which SignMuon is guaranteed to diverge. We first establish this in the momentum-free case and then observe that momentum leaves the update direction unchanged. We conjecture, supported by extensive numerical search, that no such counterexample exists for $N_{dim}\le3$; at $N_{dim} = 4$, the descent guarantee vanishes.
...
\end{equation}
Because $\langle \mathbf{G}, \mathbf{S} \rangle < 0$, the change in the objective function at every iteration is given by $f(\mathbf{W}_{t+1}) = f(\mathbf{W}_t) + 412.31 \eta$. Thus, instead of minimizing, the algorithm strictly and monotonically diverges to $+\infty$ in the momentum-free case.

\textbf{Momentum.} For the linear objective the gradient is constant, so the effective direction is a positive scaling of $\mathbf{G}$: $\tilde{\mathbf{M}}_t = \gamma_t \mathbf{G}$ for some $\gamma_t > 0$ (Standard or Nesterov). The Muon LMO is invariant to multiplication by a strictly positive scalar, so the step direction $\mathbf{D}_t = -A(\tilde{\mathbf{M}}_t) = -A(\mathbf{G})$ and hence $\mathbf{s}_t = \operatorname{sign}(\mathbf{D}_t)$ are the same at every iteration, regardless of $\mu \in [0, 1)$. The divergence above therefore holds for any momentum coefficient and both momentum variants, which completes the proof. $\blacksquare$

\subsection{Proof of Theorem~\ref{th:2} (Divergence of MuonSign)}\label{app:proof_th2}
While \cref{th:1} addresses SignMuon (sign \emph{after} the LMO), we now show that the reverse ordering--MuonSign (\cref{alg:muon_sign}), which applies the sign \emph{before} the LMO--also lacks a global convergence guarantee: we construct a strict $5\times 5$ counterexample on which MuonSign strictly diverges, for any learning rate and any momentum coefficient.
...
Let us trace the effective direction $\tilde{\mathbf{M}}_t$ in \cref{alg:muon_sign}.
...
\end{equation}
Thus, at every iteration the LMO yields the exact same step direction, namely the polar factor of the sign matrix, $\mathbf{D} = -A(\operatorname{sign}(\mathbf{G})) = \operatorname{polar}(\operatorname{sign}(\mathbf{G}))$.
...
We do so at dimension $N_{dim}=5$. Let $\mathbf{S}\in\{-1,1\}^{5\times 5}$ be the sign matrix
\begin{equation}
...
\end{pmatrix},
\end{equation}
and let $\mathbf{D}=\operatorname{polar}(\mathbf{S})=-A(\mathbf{S})$ be its polar factor, i.e.\ the Muon LMO direction MuonSign uses whenever $\operatorname{sign}(\mathbf{G})=\mathbf{S}$. Direct computation shows that $\operatorname{sign}(\mathbf{D})$ and $\mathbf{S}$ disagree at exactly one entry, $(4,2)$: there $S_{4,2}=+1$ whereas $D_{4,2}\approx -0.2425<0$, while $\operatorname{sign}(D_{i,j})=S_{i,j}$ at every other entry. We exploit this single mismatch.

Define the adversarial gradient
\begin{equation}
\mathbf{G} = \epsilon\,\mathbf{S} + (M-\epsilon)\,\mathbf{e}_4\mathbf{e}_2^\top, \qquad \epsilon>0,\; M>0,
\end{equation}
so that $G_{4,2}=\epsilon S_{4,2}+(M-\epsilon)=M>0$ and $G_{i,j}=\epsilon S_{i,j}$ otherwise; in particular $\operatorname{sign}(\mathbf{G})=\mathbf{S}$ for every $M>0$, and the LMO direction is $\mathbf{D}=\operatorname{polar}(\mathbf{S})$ at every iteration, independently of the momentum state. The descent inner product decomposes as
\begin{equation}
\langle \mathbf{G}, \mathbf{D}\rangle
= \epsilon\!\!\sum_{(i,j)\neq(4,2)}\!\! S_{i,j}D_{i,j} \;+\; M\,D_{4,2}.
\end{equation}
The first term is a fixed finite constant $C:=\sum_{(i,j)\neq(4,2)}|D_{i,j}|>0$ (all those entries agree in sign with $\mathbf{S}$), while the second term equals $M\,D_{4,2}=-0.2425\,M\to -\infty$ as $M\to\infty$. Choosing $M>C\epsilon/0.2425$ therefore yields $\langle\mathbf{G},\mathbf{D}\rangle<0$; for instance, with $\epsilon=1$ and $M=100$ one obtains $\langle\mathbf{G},\mathbf{D}\rangle\approx -13.89$, which we verified both for the exact polar factor and for the Newton--Schulz approximation used in practice.

Because $\langle\mathbf{G},\mathbf{D}\rangle<0$ and $\eta_t>0$, the objective change $-\eta_t\langle\mathbf{G},\mathbf{D}\rangle$ is strictly positive at every iteration. Thus, independent of the momentum coefficient or the scheme used, the value of MuonSign increases, and the linear target function monotonically diverges to $+\infty$. Figure~\ref{fig:muonsign_divergence} confirms this behavior: MuonSign diverges on the $\mathbf{G}$ problem, whereas the Muon, SGD, and EF21-MuonUSign methods minimize the target function. This concludes the proof. $\blacksquare$

% \newpage

% \subsection{Divergence of MuonUDSign}\label{app:proof_th3}
% TODO: validate the counterexample. It may be wrong. Final validation must contain a plot. Likely the plot will be joint with MuonSign.
...
\subsection{Convergence of EF21-MuonUSign}\label{app:hyp}
We now establish the convergence of the Error-Feedback variants. Let $\{\mathbf{X}_t\}_{t\ge 0}$ denote the sequence of global model parameters generated by the algorithm, and $f(\mathbf{X})$ the federated objective of Equation~\ref{fed_eq}.

Our analysis rests on the following standard assumptions.
...
% \end{hypothesis}
\begin{theorem}[Convergence of EF21-MuonUSign]\label{thm:conv}
Under Assumptions~\ref{as:1}--\ref{as:3} and~\ref{as:5}, the Federated EF21-MuonUSign algorithm (Algorithm~\ref{ef21_alg}) converges in the smooth non-convex setting: there exists a learning rate $\eta_t$ such that
\begin{equation}
\min_{0\le t\le T}\mathbb{E}\bigl[\|\nabla f(\mathbf{X}_t)\|_*^2\bigr] = \mathcal{O}\bigl(T^{-1/2}\bigr).
\end{equation}
The bidirectional variant EF21-MuonUDSign (Algorithm~\ref{ef21_ud_alg}) satisfies the same rate, since its additional downlink error-feedback loop is itself contractive and is covered by the two-sided analysis of \citet{gruntkowska2025error}. The key ingredients--estimator contraction (Lemma~\ref{lem:est}) and momentum tracking (Lemma~\ref{lem:mom})--are established in Appendix~\ref{app:ef_bounds}. In contrast, the non-Error-Feedback methods SignMuon and MuonSign admit no such guarantee (Theorems~\ref{th:1},~\ref{th:2}).
\end{theorem}


\begin{remark}[\textbf{Empirical quality of SignMuon}]\label{rem:signmuon_emp}
Replacing the Muon LMO direction $\mathbf{D}_t = \mathbf{U}_t \mathbf{V}_t^\top$ by its sign compression $\operatorname{sign}(\mathbf{D}_t)$ preserves the core geometry of orthonormal matrix-aware updates. Empirically, on deep-learning problems SignMuon retains a training quality close to that of full-precision Muon at a $\sim32\times$ lower uplink communication cost and outperforms the coordinate-wise SignSGD baseline in both the centralized and the federated regimes.
\end{remark}
...
MuonSign & $[1 \cdot 10^{-4}, 1 \cdot 10^{-3}]$ with step $1 \cdot 10^{-4}$ & $\{0.1, 0.2, \dots, 0.9, 0.95\}$ \\
EF21-MuonUSign & $[5 \cdot 10^{-3}, 2 \cdot 10^{-2}]$ with step $1 \cdot 10^{-4}$ & $\{0.1, 0.2, \dots, 0.9, 0.95\}$ \\
EF21-MuonUDSign & $[5 \cdot 10^{-3}, 2 \cdot 10^{-2}]$ with step $1 \cdot 10^{-4}$ & $\{0.1, 0.2, \dots, 0.9, 0.95\}$ \\
...
\cline{2-2} \cline{5-6} \cline{8-9}
& EF21-MuonUDSign & & & 0.007 & 0.001& & 99.95\% & 93.86\% \\
\hline
\end{tabular}
...
%       \State $\mathbf{d}_{t+1}^{\mathrm{est}} \gets \mathbf{d}_t^{\mathrm{est}} + \alpha_t\operatorname{sign}(\Delta_t)$ \Comment{Uplink EF21}
...
\begin{algorithm}[!h]
    ...
    \For{$t = 1$ to $T$}
        ...
        \State $\mathbf{D}_t \gets \mathrm{MuonLMO}(\mathbf{s}_t)$
...
\For{$t = 1$ to $T$}
  ...
  \State $\alpha_t \gets \operatorname{mean}(|\Delta_t|)$;
  \State $\mathbf{g}_{t+1}^{\mathrm{est}} \gets \mathbf{g}_t^{\mathrm{est}} + \alpha_t\operatorname{sign}(\Delta_t)$ \Comment{Uplink EF21}
...
\alpha_t^{\uparrow}\operatorname{sign}(\Delta_t^{\uparrow})$ \Comment{Uplink EF21}
\\
\State $\mathbf{D}_t \gets \mathrm{MuonLMO}(\mathbf{g}_{t+1}^{\mathrm{est}})$
\State $\mathbf{X}_{t+1} \gets \mathbf{X}_t - \eta_t\mathbf{D}_t$
...
\begin{algorithm}[!t]
\caption{Federated EF21-MuonUDSign}
\label{ef21_ud_alg}
