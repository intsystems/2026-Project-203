...
Our theory, like that of \citet{mishra2026signmuon}, is stated for a single matrix, where one scalar step size suffices. A network has layers of very different shapes, and the methods of \eqref{eq:three_placements} produce step matrices from \emph{two} families whose norms scale differently with shape. A single global $\eta$ therefore cannot be simultaneously correct for both families and across layers. This is not a gap between theory and practice that we must tolerate: the layer-wise LMO framework to which our convergence result reduces \citep{riabinin2025gluon,gruntkowska2025error} already carries per-layer norms $\|\cdot\|_{(\ell)}$, smoothness constants $L_\ell$ and radii. What follows instantiates that radius rather than leaving it constant.
...
\begin{aligned}
\text{\textsc{lmo} family } &(\text{Muon, MuonUSign, EF21-Muon}\{\text{USign},\text{Sign}\}): &&\mathbf{s}=\mathbf{U}\mathbf{V}^\top, \\
\text{\textsc{sign} family } &(\text{SignMuon, MuonSign, SignSGD}): &&\mathbf{s}\in\{\pm1\}^{m\times n}.
\end{aligned}
\end{equation}
Both have \emph{exactly} known Frobenius norms: $\|\mathbf{U}\mathbf{V}^\top\|_F^2=\operatorname{tr}(\mathbf{V}\mathbf{U}^\top\mathbf{U}\mathbf{V}^\top)=\operatorname{tr}(\mathbf{V}^\top\mathbf{V})=r$, so $\|\mathbf{s}\|_F=\sqrt{r}$; and a $\pm1$ matrix has $\|\mathbf{s}\|_F=\sqrt{mn}$.
