...
decrease and stay bounded. By Lemma~\ref{lem:efsm_realize} each $\mu$ and variant has its own $\tilde f$
generating the same target sequence \eqref{eq:target_seq}, so the EF21-SignMuon
trajectory shown is representative of all of them.}
...
matrices driving the divergent tail have condition number $\tfrac{16}{9}$
throughout (the two-step preamble, which only sets the phase, is not claimed to
be well conditioned).
