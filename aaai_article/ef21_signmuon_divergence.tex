...
Consider the target $\mathbf D_t$ as a $2\times2$ matrix whose off-diagonal is
...
$\alpha_t$, so the flipping off-diagonal holds $\alpha_t$ at $\Theta(1)$, which
...
of $L$.
Second, the divergence is due entirely to the estimator's response to a fixed
