...
(iv)~EF-UDSignMuon, a bidirectional variant that adds scaled-sign compression on the downlink, keeping both channels at one bit per parameter.
...
\begin{equation}
f(\mathbf{W}_{t+1}) - f(\mathbf{W}_t) = -\eta_t \lambda_{\mathrm{mult}} \langle \mathbf{G}, \mathbf{D} \rangle.
\end{equation}
...
Under Assumptions~\ref{as:1}--\ref{as:3} and~\ref{as:5}, the Federated EF-USignMuon algorithm (Algorithm~\ref{ef21_alg}) converges in the smooth non-convex setting. There exists a choice of the learning rate $\eta_t$ such that
\begin{equation}
\min_{0 \le t \le T}\mathbb{E}\bigl[\|\nabla f(\mathbf{X}_t)\|_*^2\bigr]
...
\label{central_alg}
\textbf{Input}: Initial model $\mathbf{X}_0$, initial gradient $\mathbf{G}_0$, momentum coefficient $\mu$, learning rate $\eta_t$, $\lambda_{\mathrm{mult}}=1$, number of Newton-Schulz iterations $\mathrm{ns\_steps}$ \\
...
\begin{algorithm}[!h]
    ...
    \For{$t = 0$ to $(T - 1)$}
        ...
        \begin{cases} 
            ...
            \mathbf{M}_t, & \text{(otherwise)}
        \end{cases}$
        \State $\mathbf{s}_t \gets \operatorname{sign}(\tilde{\mathbf{M}}_t)$ \Comment{Sign compression BEFORE LMO}
        \State $\mathbf{Y} \gets \text{ReshapeTo2D}(\mathbf{s}_t)$ \Comment{Flatten tensor to matrix $m \times n$}
        \State $\mathbf{Y} \gets \mathbf{Y} / \|\mathbf{Y}\|_F$ \Comment{Scale appropriately for Newton-Schulz convergence}
        \For{$k = 1$ to $\mathrm{ns\_steps}$} \Comment{Newton-Schulz orthogonalization (Muon LMO)}
            \State $\mathbf{A} \gets \mathbf{Y} \mathbf{Y}^\top$
            \State $\mathbf{Y} \gets a\,\mathbf{Y} - b\,\mathbf{A}\mathbf{Y} + c\,\mathbf{A}^2\mathbf{Y}$
        \EndFor
        \State $\mathbf{D}_t \gets \text{ReshapeToOriginal}(\mathbf{Y})$ \Comment{Reshape back to original tensor}
        \State $\mathbf{X}_{t+1} \gets \mathbf{X}_t - \eta_t \lambda_{\mathrm{mult}} \mathbf{D}_t$ \Comment{Update parameters}
    \EndFor
    \end{algorithmic}
    \end{algorithm}
    
\begin{algorithm}[!t]
\caption{Centralized EF-USignMuon}
...
\begin{algorithm}[!t]
\caption{Centralized EF-UDSignMuon (bidirectional)}
\label{central_alg_ud}
\textbf{Input}: Initial model $\mathbf{X}_0=\mathbf{W}_0$,  initial gradient $\mathbf{G}_0$, momentum coefficient $\mu$, learning rate $\eta_t$, $\lambda_{\mathrm{mult}}=1$, number of Newton-Schulz iterations $\mathrm{ns\_steps}$ \\
\textbf{Output}: Updated model $\mathbf{X}$
\begin{algorithmic}[1]
\State $\mathbf{M}_0 \gets 0,\quad \mathbf{g}_0^{\mathrm{est}} \gets 0$
\For{$t = 0$ to $(T - 1)$}
...
\State $\mathbf{Y} \gets \text{ReshapeTo2D}(\mathbf{g}_{t+1}^{\mathrm{est}})$
\For{$k = 1$ to $\mathrm{ns\_steps}$} \Comment{Newton-Schulz on the estimator}
    \State $\mathbf{A} \gets \mathbf{Y}\mathbf{Y}^\top$;\ \ $\mathbf{Y} \gets a\,\mathbf{Y} - b\,\mathbf{A}\mathbf{Y} +
c\,\mathbf{A}^2\mathbf{Y}$
\EndFor
\State $\mathbf{D}_t \gets \text{ReshapeToOriginal}(\mathbf{Y})$ \Comment{Muon LMO direction}
\State $\mathbf{X}_{t+1} \gets \mathbf{X}_t - \eta_t\lambda_{\mathrm{mult}}\mathbf{D}_t$
\State $\Delta_t^{\downarrow} \gets \mathbf{X}_{t+1} - \mathbf{W}_t$;
\quad $\alpha_t^{\downarrow} \gets
