...
\end{equation}
again with one pair of constants per layer for $g=f$ and per layer and client for $g=f_j$. The two tuples may differ in every block although only block $i$ enters the bound, and no proximity restriction on them is imposed; at $L^{1,g}_i=0$ the display is Assumption~\ref{as:2}. It is Assumptions~8--9 of \citet{gruntkowska2025error}, the hypotheses of the theorem Corollary~\ref{cor:l0l1} quotes.
...
The unit-gain multipliers of Appendix~\ref{app:lrscale} need no special case either: $\lambda=\sqrt{\mathrm{fan\text{-}out}}/\|\mathbf{P}\|_F$ returns $(1,\,1/\sqrt{n})$ for the \textsc{lmo} and \textsc{sign} families on $\mathbb{R}^{1\times n}$, and $(\sqrt{m},\,1)$ on $\mathbb{R}^{m\times1}$.
...
\end{equation}
so a counterexample with polar factor $\mathbf{Q}$ exists precisely when $\lambda_{\min}\bigl(\operatorname{sym}(\mathbf{Q}^\top\operatorname{sign}(\mathbf{Q}))\bigr)<0$: necessity because $\mathbf{H}\succ0$, sufficiency by taking $\mathbf{H}=\mathbf{w}\mathbf{w}^\top+\delta\mathbf{I}$ at a minimizing eigenvector $\mathbf{w}$ and $\delta>0$ small, which keeps $\mathbf{G}$ invertible and its polar factor unique. The question thus concerns $O(n)$ alone. Numerical search returned no negative eigenvalue over $O(2)$ or $O(3)$; over $O(4)$ it does, and the orthogonal matrix $\mathbf{O}$ below is one such $\mathbf{Q}$.
...
\bottomrule
\end{tabular}
\caption{Alignment $\rho_t$ of Equation~\eqref{eq:alignment} along the tuned trajectory, over three problem draws at $m = n = 100$. The closed form is evaluated at $\mathbf{X}_0$ without momentum and is comparable only to rows whose tuned $\mu$ is $0$; with momentum the step is built from $\mathbf{M}_t$ and $\rho_t$ falls accordingly. The six sign-around-the-\textsc{lmo} methods admit no closed form. SGD's is $1$, but its step is not instrumented, so the distribution is left empty rather than filled with the constant; Adam is omitted, its step lying outside the descent lemma. Every method of Theorems~\ref{th:1}--\ref{th:3} stays bounded away from zero. The single negative excursion is EF21-MuonSign's, which Appendix~\ref{app:hyp} proves convergent through the EF21 estimator rather than through per-step descent.}
...
One reading of the table is worth stating explicitly, because it is not the same across the three networks. On the sign-after placement, error feedback costs more under federation than in either other setting: EF21-SignMuon lies $1.00$ points below SignMuon here, against $0.29$ points centrally (Table~\ref{tab:cifar_main}), while on nanoGPT the two are indistinguishable (Table~\ref{tab:nanogpt}). What the first error-feedback loop costs is therefore setting-dependent, and this is the setting in which it is largest.
