...
The experiments were not all run on the same machine. The synthetic study ran on one NVIDIA RTX A4000 (Linux, PyTorch 2.7, CUDA 12.8); the network experiments ran on single-GPU workstations, and the language-modelling runs on a node of $8\times$H100. Every run stamps its own hardware, commit and wall time into its output directory, so the released results identify the machine each number came from; the repository documents how to regenerate the table.
...
On a deterministic $L$-smooth convex quadratic we measure the scalar the guarantees rest on: the alignment $\rho_t = \langle \nabla F(\mathbf{X}_t), \mathbf{D}_t\rangle / (\|\nabla F(\mathbf{X}_t)\|_F\|\mathbf{D}_t\|_F)$ between the gradient and the step. Theorems~\ref{th:1}--\ref{th:3} construct instances driving it negative; on random instances the three methods they cover keep $\rho_t \ge 0.142$ throughout a tuned trajectory, so the construction is not one random data reproduces. Only EF21-MuonSign goes negative, on $0.95\%$ of steps and to $-0.026$; it is also the one method here with a convergence guarantee (Appendix~\ref{app:hyp}), obtained without per-step descent.

The same experiment separates the two effects a fixed-target iteration count confounds. Sign compression of an \textsc{lmo} step buys a lower accuracy floor, not a faster rate: SignMuon's floor lies a factor $1.74$--$1.80$ below SignSGD's at every step size, and the two share $\|\mathbf{S}\|_F$ exactly, so the separation is in the floor rather than the step length. Section~\ref{app:synthetic_v1} retains the earlier comparison.
...
\bottomrule
\end{tabular}
\caption{Fixed-target criterion under the current protocol: fewest iterations to $F \le 10^{-3}$ within $T_{\max} = 5000$ at $m = n = 100$, over three problem draws, $(\eta,\mu,\text{schedule})$ tuned on five-decade logarithmic grids. A constant schedule wins for every method. Best $F$ and $\min\|\nabla F\|_F$ are minima over all $5000$ iterations rather than values at the crossing, so they report the floor the method settles into. Section~\ref{app:synthetic_v1} applies the same criterion at $m = n = 500$ on one draw; the iteration counts are not comparable across the two.}
...
\bottomrule
\end{tabular}
\caption{Floor exponent, budget exponents and stability edge against the predicted values, at $m = n = 100$ over three problem draws. The floor exponent attains its prediction exactly for all nine methods possessing a floor. The eight norm-fixed methods tune their step size as the nonconvex bound prescribes, $q$ falling in $[0.39, 0.55]$ about the predicted $1/2$ at $R^2 \ge 0.94$, but converge faster than either bound admits: $p \approx 2$ at $R^2 \ge 0.99$, twice the strongly convex value, a quadratic being easier than the worst case of its smoothness class. SGD has no floor, its step vanishing with the gradient, so its floor columns are empty by construction and its $\eta_{\max}$ is the control reproducing $2/L = 2.019$; neither baseline admits a power law in $\eta^\star$. $^\dagger$~SGD has no $\|\mathbf{S}\|_F$; the entry is $\eta_{\max}$, $1.02$ times $2/L$. $^\ddagger$~The search reached its ceiling without an edge: Adam's step is bounded by roughly $\eta$ regardless of the gradient, so it oscillates rather than diverges.}
...
Learning rates are searched on logarithmic grids spanning five decades, one per step-norm family, each ending past the largest stability edge measured for that family so that no stable step size falls outside the search. The one-decade linear grids of Section~\ref{app:synthetic_v1} were narrow enough to censor optima at a grid boundary, which is an upper bound rather than a tuned value; the search flags any optimum at an edge. No learning rate reported above is flagged. The windows are derived from $\|\mathbf{S}\|_F$ rather than from the method name, which matters for MuonUSign and EF21-SignMuon: both take a step of length $\sqrt{r}$ despite the \textsc{sign} in their names, and under a window assigned by name both were searched a decade below the range their step length calls for.
...
\paragraph{Sensitivity: what the rule can and cannot affect.} Equation~\eqref{eq:unit_gain_main} is a heuristic, so it is worth being precise about its scope. Each candidate rule --- one global rate ($\lambda_\ell=1$), unit gain ($\lambda_\ell=1/\sqrt{\text{fan-in}}$), $\mu$P ($\lambda_\ell=1/\text{fan-in}$) --- shifts the \emph{selected} $\eta_0$ by roughly the multiplier it prescribes, which is what it is for; what would matter is whether it shifts the \emph{ordering} of the methods. Two things bound the exposure. The \textsc{lmo} family is unaffected: unit gain and $\mu$P prescribe the identical multiplier for it, so five of the eight methods, including every one that carries a guarantee, cannot move. And within the \textsc{sign} family every method is tuned and reported under the \emph{same} rule, so a wrong exponent rescales all three alike rather than favouring one. What we have not done is re-tune the \textsc{sign} family from scratch under the two competing conventions and confirm the ordering directly; that ablation --- on federated CNN2, whose three matrix parameters span a factor of $7.8$ in $\lambda_\ell^{\textsc{sign}}$ with no single shape dominating the parameter count, and where a wrong exponent therefore has the most scope to show --- remains outstanding, and we flag the sign-family ordering as resting on the rule to that extent.
