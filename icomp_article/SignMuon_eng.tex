...
% \end{theorem}
\begin{theorem}\label{th:1} There exist a function $f(\mW)$ and a matrix dimension $N \geq 4$ such that the SignMuon algorithm diverges.
\end{theorem}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{images/signmuon_divergence.pdf}
    \caption{Comparison of optimization trajectories for the linear objective function $f(\mW) = \mathrm{Tr}(\mG^\top \mW)$, demonstrating the strict divergence of SignMuon, while other methods successfully minimize the function.}
...
\begin{algorithm}[H]
\caption{Federated EF Sign-Muon}
\begin{algorithmic}[1]
\Require Initial model $\mathbf{X}_0$, clients number $N$, rounds number $\mathrm{rounds}$, local steps number $\mathrm{n\_steps}$, learning rate for the main layers $\eta_t$, learning rate for the last layer $\eta_t^{\text{aux}}$,  momentum coefficient $\mu$
\Ensure Updated global model $\mathbf{X}$
...
\paragraph{Proof} The proof is constructed using a counterexample that shows the existence of a smooth function and a matrix dimension $N \ge 4$ for which the SignMuon algorithm is guaranteed to diverge. Extensive numerical searches strongly suggest that no counterexamples exist for $N \le 3$ and SignMuon should converge. However, at $N = 4$, this convergence guarantee vanishes.
...
We construct the gradient matrix $\mathbf{G}$ by assigning a massive singular value ($\sigma_1 = 1000$) to this pathological component and a singular value of $1$ to the rest of the orthogonal space:
\begin{equation}
