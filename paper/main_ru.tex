...
\begin{algorithmic}[1]
\Require Начальная модель $\mathbf{X}_0$, число клиентов $M$, число раундов $\mathrm{rounds}$, число локальных шагов $\mathrm{n\_steps}$, шаг обучения $\eta_t$ (для основных слоёв), шаг обучения $\eta_t^{\text{aux}}$ (для последнего слоя), коэффициент моментума $\mu$, коэффициент весового распада $\lambda$
\Ensure Обновлённая глобальная модель $\mathbf{X}$

\State $\mathbf{G}_0 \gets 0$ \Comment{Инициализация глобальной оценки градиента на сервере (только для не-last\_layer параметров)}
\For{$m = 1$ to $M$} 
    \State $\mathbf{g}_0^{(m)} \gets 0, \; \mathbf{M}_0^{(m)} \gets 0$ \Comment{Инициализация локальных буферов на клиентах}
\EndFor
\\
\For{$t = 0$ to $\mathrm{rounds} - 1$}

    \For{$m = 1$ to $M$ \textbf{in parallel}} \Comment{Локальный шаг EF}
        \State Вычислить накопленный градиент по $\mathrm{n\_steps}$ батчам с учётом $\lambda$: $\nabla f_{\lambda}(\mathbf{X}_t^{(m)}; \xi^{(m)})$
        \State $\mathbf{M}_t^{(m)} \gets \mu \mathbf{M}_{t-1}^{(m)} + \frac{1}{\mathrm{n\_steps}} \sum_{i=1}^{\mathrm{n\_steps}} \nabla f_{\lambda}(\mathbf{X}_t^{(m)}; \xi_{t,i}^{(m)})$
        \State $\Delta_t^{(m)} \gets \mathbf{M}_t^{(m)} - \mathbf{g}_t^{(m)}$ \Comment{Вычисление невязки}
        \State $\alpha_t^{(m)} \gets \operatorname{mean}(|\Delta_t^{(m)}|)$
        \State $\mathbf{s}_t^{(m)} \gets \operatorname{sign}(\Delta_t^{(m)})$
        \State $\mathbf{g}_{t+1}^{(m)} \gets \mathbf{g}_t^{(m)} + \alpha_t^{(m)} \mathbf{s}_t^{(m)}$
        \State Отправить $(\mathbf{s}_t^{(m)}, \alpha_t^{(m)})$ на сервер
    \EndFor
    \\
    \State $\mathbf{G}_{t+1} \gets \mathbf{G}_t + \frac{1}{M} \sum_{m=1}^M \alpha_t^{(m)} \mathbf{s}_t^{(m)}$ \Comment{Глобальное обновление на сервере}
    \\
    \For{\textbf{each} параметр $\mathbf{X}$ \textbf{not in last\_layer}} \Comment{Обновление основных слоёв через Muon}
        \State $\mathbf{Y} \gets \text{ReshapeTo2D}(\mathbf{G}_{t+1})$
        \State $\mathbf{Y} \gets \mathbf{Y} / (\|\mathbf{Y}\|_F + \varepsilon)$
    ...
    \EndFor
    \\
    \For{\textbf{each} параметр $\mathbf{X}^{\text{last}}$ \textbf{in last\_layer}} \Comment{Обновление последнего слоя через AdamW}
        \State $\nabla^{\text{last}} \gets \frac{1}{M} \sum_{m=1}^M \mathbf{s}_t^{(m)}$ \Comment{Усреднение сырых невязок}
        \State $\text{AdamW}(\mathbf{X}^{\text{last}}, \nabla^{\text{last}}, \eta_t^{\text{aux}}, \lambda)$
    \EndFor
\EndFor
\State \Return $\mathbf{X}_{\mathrm{rounds}}$
\label{ef21_alg}
...
Для обеспечения строгих теоретических доказательств сходимости мы опираемся на недавнюю работу Gruntkowska \cite{gruntkowska2025}, в которой фреймворк марковских невязок адаптирован для семейства LMO-оптимизаторов. Следуя предложенной архитектуре \cite{gruntkowska2025error}, мы реализовали модификацию алгоритма Federated EF-SignMuon. В данной реализации процедура ортогонализации (LMO) переносится на сторону сервера. Клиенты не вычисляют LMO локально; вместо этого они применяют знаковую компрессию к разности между обновленным локальным моментумом и своей текущей оценкой градиента. Сервер агрегирует полученные невязки, восстанавливает оценку градиента и только после этого применяет к ней оператор Muon. Полный алгоритм представлен в \ref{ef21_alg}.
