...
В федеративном сценарии использования алгоритма SignMuon каждый клиент независимо вычисляет знаковое LMO-направление, а сервер выполняет агрегацию и обновление глобальной модели. Каждый клиент сначала копирует глобальную модель, после этого делает $\mathrm{n\_steps}$ локальных шагов стохастической оптимизации и на каждом шаге вычисляет градиент по текущему батчу. Все локальные градиенты суммируются, после чего усредняются, данный процесс повторяется до предела количества раундов $\mathrm{rounds}$:
$$
\mathbf{G}_t^{(m)}=\frac{1}{n_{\text{steps}}}\sum_{i=1}^{n_{\text{steps}}}\nabla f(\mathbf{X}_t;\xi_{t,i}^{(m)}).
$$
$$
\mathbf{s}_t^{(m)} = \operatorname{sign}\bigl(\mathbf{D}_t^{(m)} \bigr) 
= \operatorname{sign}\bigl(A(\mathbf{G}_t^{(m)})\bigr) \in \{\pm 1\}^{m \times n},
$$
...
Операция majority-vote гарантирует, что результирующий знак $\mathbf{s}_t^{\mathrm{agg}}$ равен $+1$ или $-1$ в каждой компоненте и соответствует «мнению большинства» клиентов. 

Далее сервер применяет momentum уже к агрегированному знаку:
\[
\mathbf{M}_t = \mu \mathbf{M}_{t-1} + (1-\mu) \mathbf{s}_t^{\mathrm{agg}},
\]
где $\mathbf{M}_t$ — буфер моментума на сервере. Обновление глобальной модели для матричных параметров (кроме последнего слоя) выполняется по правилу:
\[
\mathbf{X}_{t+1} = \mathbf{X}_t - \eta_t \cdot \operatorname{sign}(\mathbf{M}_t).
\]
...
\State $\mathbf{M}_0 \gets 0$ \Comment{Инициализация буфера моментума}

\For{$t = 0$ to $\mathrm{rounds} - 1$}

    \For{$m = 1$ to $M$} \Comment{Параллельно на каждом клиенте}
        \State $\mathbf{X}_t^{(m)} \gets \mathbf{X}_t$ \Comment{Получение глобальной модели}
        \\
        \State $\mathbf{G}_t^{(m)} \gets \frac{1}{\mathrm{n\_steps}} \sum_{i=1}^{\mathrm{n\_steps}} \nabla f(\mathbf{X}_t^{(m)}; \xi_{t,i}^{(m)})$ \Comment{Усреднённый градиент}
        \\
        \State $\mathbf{Y}_t^{(m)} \gets \text{ReshapeTo2D}(\mathbf{G}_t^{(m)})$ \Comment{Приведение тензора к матрице $m \times n$}
        \\
        \State $\mathbf{Y}_t^{(m)} \gets \mathbf{Y}_t^{(m)} / (\|\mathbf{Y}_t^{(m)}\|_F + \varepsilon)$ \Comment{Нормализация по норме Фробениуса}
        \\
        \For{$k = 1$ to $\mathrm{ns\_steps}$} \Comment{Итерации Ньютона-Шульца (Muon)}
        ...
        \EndFor
        \\
        \State $\mathbf{D}_t^{(m)} \gets \text{ReshapeToOriginal}(\mathbf{Y}_t^{(m)})$ \Comment{Восстановление размерности}
        \\
    ...
    \EndFor
    \\
    \State $\mathbf{s}_t^{\mathrm{agg}} \gets \operatorname{sign}\!\left(\sum_{m=1}^{M} \mathbf{s}^{(m)}\right)$ \Comment{Majority-vote агрегация на сервере}
    \\
    \State $\mathbf{M}_t \gets \mu \mathbf{M}_{t-1} + (1-\mu)\mathbf{s}_t^{\mathrm{agg}}$ \Comment{Обновление моментума}
    \\
    \State $\mathbf{X}_{t+1} \gets \mathbf{X}_t - \eta_t \cdot \operatorname{sign}(\mathbf{M}_t)$ \Comment{Обновление матричных параметров}
    \\
    \State $\mathbf{X}_{t+1}^{\text{last}} \gets \text{AdamW}(\mathbf{X}_t^{\text{last}}, \mathbf{s}_t^{\mathrm{agg}})$ \Comment{Обновление последнего слоя}
\EndFor
...
\paragraph{Assumption 1.} Функция $F(\mathbf{x})$ является $L$-гладкой относительно нормы $\|\cdot\|$, т.е. для любых $\mathbf{x}, \mathbf{y} \in \mathbb{R}^d$ выполняется:
\begin{equation}
...
\begin{itemize}
    ...
    \mathbb{E}{\xi}\bigl[\|\nabla f(\mathbf{x};\xi) - \nabla f(\mathbf{x})\|^2\bigr] \le \sigma^2, \quad \sigma > 0
    \end{equation}
\end{itemize}



\paragraph{Гипотеза 1 (Сходимость).} При выполнении условий Assumption 1 и Assumption 2 алгоритм SignMuon сходится в гладком невыпуклом режиме. В частности, предполагается, что выполняется та же оценка скорости сходимости, что и для SignSGD:
\begin{equation}
\min_{0 \le t \le T}\mathbb{E}\bigl[\|\nabla F(\mathbf{x}_t)\|^2\bigr]
=
\mathcal{O}\bigl(T^{-1/2}\bigr).
\end{equation}
Иными словами, знаковая компрессия LMO-направления не ухудшает стандартный порядок сходимости в гладком невыпуклом случае.

\paragraph{Гипотеза 2 (Сохранение преимуществ Muon).}
Переход от LMO-направления Muon $\mathbf{D}_t=\mathbf{U}_t\mathbf{V}_t^\top$ к его знаковой компрессии $\operatorname{sign}(\mathbf{U}_t\mathbf{V}_t^\top)$ сохраняет основную геометрию ортонормированных обновлений. В результате SignMuon должен сохранять качество обучения, близкое к полному Muon, при существенно меньших коммуникационных затратах и превосходить SignSGD как в централизованном, так и в федеративном режимах.
...
\begin{figure}[H]
    ...
    \caption{Сравнение сходимости и точности различных алгоритмов оптимизации на обучении классификатора MNIST.}
...
\begin{figure}[H]
    ...
    \caption{Сравнение сходимости и точности различных алгоритмов оптимизации на обучении классификатора CIFAR-10.}
    \label{fig:cifar_results}
\end{figure}


\paragraph{Федеративный сценарий.}
